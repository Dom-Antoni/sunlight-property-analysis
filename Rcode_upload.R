...
#### Hier die den gewünschten Längen und Breitengrad angeben
sun <- getSunlightPosition(
  date = times,
  lat = "#latitude of your property",
  lon = "#longitude of your property"
)
# sonnenhöhe und winkel in Grad umwandeln 
...
#### Tabelle einlesen----
objekte <- read.csv("Objekte.csv",
                    fileEncoding = "latin1",
                    check.names = FALSE) %>%
  mutate(
    Länge = as.numeric(Länge),
    Breite = as.numeric(Breite),
    Höhe = as.numeric(Höhe),
...
make_shadow <- function(obj, sun) {
  ...
  shadow_length <- obj$Höhe / tan(sun$altitude)
